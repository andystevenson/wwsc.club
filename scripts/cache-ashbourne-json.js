...
const toJson = async () => {
  try {
    ...
    if (!csvStat) throw Error(`cache-ashbourne needs to be run first...`)
    ...
    if (cacheOutOfDate) {
      try {
        ...
        log.error(`cache-ashbourne-json failed because [${error}]`)
      }
    }
    ...
    log.error(`failed to cache-ashbourn-json because [${error.message}]`)
    exit(1)
  }
}
